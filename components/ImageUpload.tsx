...
interface Props {
  value: string;          // current base64/dataUrl
...
export default function ImageUpload({ value, onChange, label = 'Upload Gambar', aspectRatio = '16/9', maxSizeMB = 2 }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (data.success) { onChange(data.url); toast.success('Gambar berhasil diupload!'); }
      else toast.error(data.message || 'Gagal upload.');
    ...
    <div>
      ...
      <div
        className={`relative w-full rounded-2xl border-2 border-dashed transition-all cursor-pointer overflow-hidden ${
          dragging ? 'border-brand bg-brand/5' : value ? 'border-gray-200' : 'border-gray-200 hover:border-brand/40 bg-gray-50 hover:bg-brand/5'
        }`}
        style={{ aspectRatio }}
        ...
        {value ? (
          <>
            ...
            <div className="absolute inset-0 bg-black/0 hover:bg-black/30 transition-all flex items-center justify-center opacity-0 hover:opacity-100">
              <p className="text-white text-sm font-medium bg-black/50 px-3 py-1.5 rounded-lg">Ganti Gambar</p>
            </div>
            <button
              type="button"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-gray-400">
            {loading ? (
              ...
              <>
                ...
                <p className="text-sm font-medium">{dragging ? 'Lepaskan di sini' : 'Klik atau drag gambar'}</p>
