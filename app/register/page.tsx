...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Registration Initiated. Awaiting Verification.');
      setStep('otp');
  ...
  const handleVerify = async () => {
    const code = otp.join('');
    if (code.length !== 6) { toast.error('Required: 6-digit decryption key.'); return; }
    setLoading(true);
    try {
  ...
  const handleResend = async () => {
    ...
    const data = await res.json();
    if (data.success) toast.success('New key transmitted!'); else toast.error(data.message);
  };
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex flex-col items-center gap-2 group">
            ...
            <div>
              ...
              <p className="font-mono text-xs text-red-500 tracking-widest mt-1">{'>>'} SYS.REGISTER_NODE</p>
            </div>
          </Link>
        </div>
            ...
            {step === 'form' ? (
              <>
                <div className="mb-8 border-b border-zinc-800 pb-4">
                  <h2 className="text-lg font-black text-white uppercase tracking-widest flex items-center gap-2">
                    <User size={18} className="text-red-500" /> Initialize_User
                  </h2>
                  <p className="text-[10px] font-mono text-zinc-500 mt-1 uppercase tracking-widest">Create credentials to access database</p>
                </div>
                ...
                <form onSubmit={handleRegister} className="space-y-5">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input className="w-full bg-zinc-950 border border-zinc-800 focus:border-red-500 focus:shadow-[0_0_10px_rgba(220,38,38,0.2)] text-white font-mono text-sm pl-11 pr-4 py-3 outline-none transition-all placeholder-zinc-700" 
                        placeholder="user_identity" value={form.username}
                  ...
                  <button type="submit" disabled={loading} className="w-full py-4 mt-2 bg-red-600 hover:bg-red-500 text-white font-mono font-bold uppercase tracking-widest border border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.3)] transition-all flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed">
                    {loading ? <span className="spinner border-white/30 border-t-white" /> : <><span>Execute_Registry</span><ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" /></>}
                  </button>
                </form>
                ...
                <div className="mt-6 border-t border-zinc-800 pt-6 text-center">
                  <p className="font-mono text-xs text-zinc-500">
                    Already registered?{' '}
                    <Link href="/login" className="text-red-500 font-bold hover:text-red-400 hover:underline uppercase tracking-widest ml-1">Establish_Link</Link>
                  </p>
                </div>
              </>
            ) : (
              <>
                <div className="mb-8 text-center border-b border-zinc-800 pb-6">
                  <div className="w-16 h-16 bg-red-950/30 border border-red-900 rounded-none flex items-center justify-center mx-auto mb-4 relative overflow-hidden">
                    <div className="absolute inset-0 bg-red-500/10 animate-pulse" />
                    <TerminalSquare size={28} className="text-red-500 relative z-10" />
                  </div>
                  <h2 className="text-lg font-black text-white uppercase tracking-widest">Verify_Comm_Link</h2>
                  <p className="text-xs font-mono text-zinc-400 mt-2 leading-relaxed">
                    6-digit decryption key transmitted to:<br />
                ...
                <button onClick={handleVerify} disabled={loading} className="w-full py-4 bg-red-600 hover:bg-red-500 text-white font-mono font-bold uppercase tracking-widest border border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.3)] transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                  {loading ? <span className="spinner border-white/30 border-t-white" /> : 'Authenticate_Key'}
                </button>
