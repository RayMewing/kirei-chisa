...
export default function BannerSlider() {
  ...
  if (banners.length === 0) {
    return (
      <div className="w-full bg-gradient-to-br from-brand/10 to-red-50 rounded-2xl border border-brand/20 flex items-center justify-center" style={{ paddingBottom: '56.25%', position: 'relative' }}>
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden shadow-md group" style={{ aspectRatio: '16/9' }}>
      {banners.map((b, i) => (
        <Link
          key={b._id} href={b.linkUrl || '#'}
          className={`absolute inset-0 transition-all duration-700 ${i === current ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
        >
          <Image src={b.imageUrl} alt={b.title} fill className="object-cover" sizes="(max-width:768px) 100vw, 900px" priority={i === 0} />
      ...
      {banners.length > 1 && (
        <>
          <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 bg-black/40 hover:bg-black/60 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
            <ChevronLeft size={18} />
          </button>
          <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 bg-black/40 hover:bg-black/60 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
